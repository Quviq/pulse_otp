...
%% %CopyrightBegin%
%%
%% This file is a copy of the Erlang OTP library "application_master"
...
-include("pulse_application_master.hrl").
...
start_link(ApplData, Type) ->
    Parent = whereis(pulse_application_controller),
...
start_it(State, Type) ->
    Tag = make_ref(),
    Pid = spawn_link(pulse_application_master, start_it, [Tag, State, self(), Type]),
...
start_it(Tag, State, From, Type) ->
    ...
    case {ApplData#appl_data.phases, ApplData#appl_data.mod} of
        ...
        {Phases, {pulse_application_starter, [M, A]}} ->
